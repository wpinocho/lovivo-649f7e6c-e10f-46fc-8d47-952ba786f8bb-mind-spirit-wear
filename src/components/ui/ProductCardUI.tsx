...
export const ProductCardUI = ({ product }: ProductCardUIProps) => {
  return (
    <HeadlessProductCard product={product}>
      {(logic) => (
        <Card className="group bg-card border hover:shadow-lg transition-shadow duration-300">
            ...
            <Link to={`/products/${logic.product.slug}`} className="block">
              <div className="aspect-square bg-muted rounded-md mb-3 overflow-hidden relative">
                ...
                <div className="absolute top-2 left-2 flex flex-col gap-1">
                  ...
                  {!logic.inStock && (
                    <span className="bg-muted text-muted-foreground text-xs px-2 py-1 rounded-full font-medium shadow-sm">
                      Out of stock
                    </span>
                  )}
                </div>
